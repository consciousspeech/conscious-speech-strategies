...
export default function MindBodySpeechUpdateInfo() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      const res = await fetch(`https://formspree.io/f/${FORMSPREE_ID}`, {
        ...
        body: JSON.stringify({
          _subject: `📝 MBS Update Info: ${form.childName}`,
          ...
          "Pediatrician Name": form.pediatricianName || "Not provided",
          "Pediatrician Phone": form.pediatricianPhone || "Not provided",
          "Insurance Provider": form.insuranceProvider || "Not provided",
          "Insurance Policy #": form.insurancePolicy || "Not provided",
      ...
      <div className="mx-auto max-w-2xl px-6 pt-28 pb-20 lg:px-8">
        <div className="mb-10 text-center">
          ...
          <p className="font-body text-sm leading-relaxed text-charcoal-light">
            Already registered for Mind. Body. Speech.? We&apos;ve added a few
            new questions to keep your child safe and supported. Please take
            a couple minutes to fill these out before camp begins. No payment
            needed &mdash; you&apos;re already signed up.
          </p>
        </div>
        ...
        {done ? (
          <div className={sectionClass + " text-center"}>
            ...
            <p className="font-body text-sm text-charcoal-light">
              We&apos;ve got your updated info. See you at camp!
            </p>
            <Link
              href="/camps/mind-body-speech"
              className="mt-6 inline-block font-body text-sm font-semibold text-sage-dark hover:text-sage"
            >
              &larr; Back to Mind. Body. Speech.
            </Link>
          </div>
        ) : (
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className={sectionClass + " space-y-5"}>
              ...
              <div>
                <label className={labelClass}>Diagnoses</label>
                <textarea rows={2} className={inputClass + " resize-none"} value={form.diagnoses}
                  onChange={(e) => update("diagnoses", e.target.value)} placeholder="e.g. speech delay, autism, ADHD, etc." />
              </div>
              <div>
              ...
              <div>
                <label className={labelClass}>Has your child been evaluated for speech/language challenges?</label>
                <div className={radioGroupClass}>
                  {["Yes", "No", "Not sure"].map((opt) => (
                    <button key={opt} type="button" onClick={() => update("speechEvaluation", opt)}
                      className={form.speechEvaluation === opt ? radioActiveClass : radioClass}>{opt}</button>
                  ))}
                </div>
              </div>
              <div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  <label className={labelClass}>Pediatrician name</label>
                  <input type="text" className={inputClass} value={form.pediatricianName}
                    onChange={(e) => update("pediatricianName", e.target.value)} />
                </div>
                <div>
                  <label className={labelClass}>Pediatrician phone</label>
                  <input type="tel" className={inputClass} value={form.pediatricianPhone}
                    onChange={(e) => update("pediatricianPhone", e.target.value)} />
                </div>
              </div>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  <label className={labelClass}>Health insurance provider</label>
                  <input type="text" className={inputClass} value={form.insuranceProvider}
                    onChange={(e) => update("insuranceProvider", e.target.value)} />
                </div>
                <div>
                  <label className={labelClass}>Policy / member number</label>
                  <input type="text" className={inputClass} value={form.insurancePolicy}
